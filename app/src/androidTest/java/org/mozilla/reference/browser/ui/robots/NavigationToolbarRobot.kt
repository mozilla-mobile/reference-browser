...
class NavigationToolbarRobot {
    ...
    fun verifyReaderViewDetected(visible: Boolean = false) = assertReaderViewDetected(visible)
    ...
    class Transition {
        ...
        fun clickReaderViewIcon(interact: ReaderViewRobot.() -> Unit): ReaderViewRobot.Transition {
            mDevice.findObject(UiSelector()
                .resourceId("$packageName:id/mozac_browser_toolbar_page_actions"))
                .waitForExists(waitingTime)

            readerViewIcon().click()
            mDevice.waitForIdle()
...
private fun readerViewIcon() = onView(withParent(withId(R.id.mozac_browser_toolbar_page_actions)))
...
private fun assertReaderViewDetected(visible: Boolean) {
    mDevice.waitForIdle()
    mDevice.findObject(UiSelector()
        .description("Enable Reader View")).waitForExists(waitingTime)

    onView(
        allOf(
            withParent(withId(R.id.mozac_browser_toolbar_page_actions)),
            withContentDescription("Enable Reader View")
        )
    ).check(
