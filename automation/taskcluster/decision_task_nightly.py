...
def generate_signing_task(build_task_id, apks):
    ...
    index = "index.project.mobile.reference-browser.nightly.latest"
    routes.append(index)
...
def generate_push_task(signing_task_id, apks, commit):
    artifacts = []
    for apk in apks:
        artifacts.append("public/{}".format(os.path.basename(apk)))
